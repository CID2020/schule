...
class Application():
  def __init__(self, master):
    ...
    self.v1=[] #list of list [true/false given or not][number]
    self.ns=self.nw=self.fehler=0  #number of side, winkel
    self.lt=[] #list of text (latex)
  ...
  def generate(self):
    ...
    if a1==0: #sss
      ...
      #it could be in ssw case unsolvable, just make it sws
        ...
        random.shuffle(wl)
    self.show(sl[0],sl[1],sl[2],wl[0],wl[1],wl[2])
  ...
  def summe(self,n1,w1,w2):
    ...
    self.lt.append(tempt)
    return w3
  def show(self,a,b,c,alpha,beta,gamma):
    templ1=[a,b,c,alpha,beta,gamma]
    for n1 in range(len(self.le)):
      if not len(self.le[n1].get()) and not math.isnan(templ1[n1]):
        # second condition is used to generate homework
        if n1<3: #side
          self.le[n1].insert(0,round(templ1[n1],r4))
        else:
          self.le[n1].insert(0,round(math.degrees(templ1[n1])))
    self.latex1(self.lt)
  def calculate(self):
    self.lf.config(text = 'calculation:')
    templ=[]
    for n1 in self.v1:
      templ.append(n1[1])
    a,b,c,alphade,betade,gammade=templ
    ...
    if self.ns+self.nw>3:
      ...
      if self.v1[0][0]^self.v1[3][0] and self.v1[1][0]^self.v1[4][0]:#sws
        if math.isnan(a):
    ...
    if self.fehler:
      ...
      self.fehler=0
    else:self.show(a,b,c,alpha,beta,gamma)
    self.ns=self.nw=templ=0
    self.v1=[]
    self.lt=[]
  def anwser(self):
    self.v1=[]
    ...
    for n1 in range(len(self.le)):
      str1=self.le[n1].get()
      if not len(str1):
        self.v1.append([False,float('nan')])
      else:
        try:num1=float(str1)
        except ValueError:
          self.fehler=1
